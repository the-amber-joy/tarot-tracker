<script lang="ts">
  ...
  function handleSubmit(e: Event) {
    ...
    if (cardName.trim() && tarotCards.length > 0) {
      const isValidCard = tarotCards.some(
        (card) => card.name === cardName.trim(),
      );
      if (!isValidCard) {
        validationMessage = "Please select a valid card name from the list.";
  ...
  function validateAndSetCard() {
    ...
    const exactMatch = tarotCards.find(
      (card) => card.name.toLowerCase() === searchInput.toLowerCase(),
    );
    if (exactMatch) {
  ...
  function handleSearchBlur(event: FocusEvent) {
    ...
    if (
      ...
      (relatedTarget.classList.contains("dropdown-item") ||
        relatedTarget.classList.contains("btn-clear-search"))
    ) {
      return;
    }
